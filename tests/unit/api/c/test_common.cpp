...
TEST(TestCommonApi, EventToStringFunctionsWork)
{
  ...
  EXPECT_TRUE(rdmnet_dynamic_uid_status_to_string(kRdmnetDynamicUidStatusOk) != nullptr);
  EXPECT_TRUE(rdmnet_dynamic_uid_status_to_string(static_cast<rdmnet_dynamic_uid_status_t>(INT_MAX)) == nullptr);
  EXPECT_TRUE(rdmnet_dynamic_uid_status_to_string(static_cast<rdmnet_dynamic_uid_status_t>(-1)) == nullptr);
}
